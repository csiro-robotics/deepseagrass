...
class DataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        images = [cv2.cvtColor(cv2.imread(os.path.join(path, self.images_paths[k])), cv2.COLOR_BGR2RGB) for k in indexes]             
...
def loadData():
    ...
    for category in range(len(CLASSES)):
        img_list = [f for f in os.listdir(os.path.join(TRAIN_PATH, CLASSES[category])) if re.match(r'^(?![\._]).*$', f)] # filter out the apple files
    ...
    for category in range(len(CLASSES)):
        img_list = [f for f in os.listdir(os.path.join(VAL_PATH, CLASSES[category])) if re.match(r'^(?![\._]).*$', f)] # filter out the apple files
    ...
    train_data = DataGenerator(train_images_array, train_labels_array, batch_size=BATCH_SIZE, augment=True, shuffle=True)
    val_data = DataGenerator(val_images_array, val_labels_array, batch_size=BATCH_SIZE, augment=False, shuffle=True, validate=True) 
...
if __name__ == "__main__": 
    ...
    # VGG16
    # baseModel = VGG16(weights="imagenet", include_top=False, input_tensor=Input(shape=CROP_SHAPE), pooling='avg', layers=tf.keras.layers)
