...
def loadData():
    ...
    for category in range(len(CLASSES)):
        img_list = [f for f in os.listdir(os.path.join(TEST_PATH, CLASSES[category])) if re.match(r'^(?![\._]).*$', f)] # filter out the apple files
...
if __name__ == "__main__":
    ...
    parser.add_argument('--metrics', action="store", type=bool, default=True)
    parser.add_argument('--save_incorrect', action="store", type=bool, default=False)
    parser.add_argument('--visualise_inferences', action="store", type=bool, default=False)
    ...
    if args.visualise_inferences == True:
        print("[INFO] generating output images...")
        TEST_PATH = 'Test-Visualisation'
        test_data, test_images_array = loadData("test")
